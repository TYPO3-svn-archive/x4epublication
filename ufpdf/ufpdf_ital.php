...
else if (file_exists('libs/fpdf.php')) include_once 'libs/fpdf.php';
...
class FLOFPDF extends UFPDF
{
    ...
    {
	$cp=$this->utf8_to_codepoints(trim($s));
        ...
        for ( $i = 0; $i < $this->strlen( $s ); $i++ )
        {
		  ...
		  //$c = $s{ $i };
            ...
            $cw = $this->CurrentFont[ 'cw' ][$cp[$i] ] * ( $this->FontSizePt / 1000 );
            ...
            if ( $contentWidth + $cw > $maxWidth )
            {
                ...
                if( $align == 'J' )
                {
                    ...
                    foreach ( $content as $chunk )
                        $numchar += $this->strlen( $chunk);

                    // if there's more than one space, find word spacing in points
                    if ( $numchar > 0 )
                        $ecart = ( $maxWidth - $contentWidth ) / $numchar;
                    else
                        $ecart = 0;

		    //echo $numchar;
                    $this->_out( sprintf( 'BT %.3f Tc ET', $ecart ) );

                }

                // otherwise, we want normal spacing
                else
                    $this->_out( sprintf( 'BT %.3f Tc ET', 0 ) );
                ...
                foreach ( $content as $k => $chunk )
                {
                    ...
                    $stringWidth = $this->GetStringWidth( $chunk ) + ( $ecart * $this->strlen( $chunk ) / $this->k );
            ...
            else
            {
                ...
                $contentWidth += $cw;
                $currContent .= $this->code2utf($cp[$i]);;
